...
SELECT COUNT() AS total_passengers FROM titanic; SELECT Survived, COUNT() AS count FROM titanic GROUP BY Survived;

SELECT Pclass, COUNT(*) AS total_in_class FROM titanic GROUP BY Pclass;

SELECT AVG(Age) AS avg_age FROM titanic;

SELECT Sex, Survived, COUNT(*) AS count FROM titanic GROUP BY Sex, Survived;
